...
public sealed class AuthorizationPolicyTests
{
    ...
    [Fact]
    public async Task UserAdministrationPolicies_SeparateViewAndManagePermissions()
    {
        Guid userId = Guid.NewGuid();
        Guid organizationId = Guid.NewGuid();
        var accessor = new HttpContextAccessor { HttpContext = new DefaultHttpContext() };
        ...
        accessor.HttpContext.Items[ManagementAuthenticationConstants.SessionItemKey] = CreateSession(
            userId, organizationId, [PermissionCodes.ManagementAccess, PermissionCodes.UsersView], null);
        Assert.True((await authorization.AuthorizeAsync(principal, null, KalmPolicies.UserAdministrationView)).Succeeded);
        Assert.False((await authorization.AuthorizeAsync(principal, null, KalmPolicies.UserAdministrationManage)).Succeeded);

        accessor.HttpContext.Items[ManagementAuthenticationConstants.SessionItemKey] = CreateSession(
            userId, organizationId, [PermissionCodes.ManagementAccess, PermissionCodes.UsersManage], null);
        Assert.False((await authorization.AuthorizeAsync(principal, null, KalmPolicies.UserAdministrationView)).Succeeded);
        Assert.True((await authorization.AuthorizeAsync(principal, null, KalmPolicies.UserAdministrationManage)).Succeeded);
    }
