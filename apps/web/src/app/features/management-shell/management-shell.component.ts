...
import { AUDIT_VIEW_PERMISSION, BRANCHES_VIEW_PERMISSION, CATALOG_VIEW_PERMISSION, DEVICES_MANAGE_PERMISSION, ROLES_MANAGE_PERMISSION, USERS_VIEW_PERMISSION } from "../../core/auth/management-permissions";
...
export class ManagementShellComponent {
  ...
  protected readonly canViewAudit = computed(() => this.auth.hasPermission(AUDIT_VIEW_PERMISSION));
  protected readonly canViewCatalog = computed(() => this.auth.hasPermission(CATALOG_VIEW_PERMISSION));
  ...
  protected readonly auditLabel = computed(() => this.language.language() === "ar" ? "سجل التدقيق" : "Audit log");
  protected readonly catalogLabel = computed(() => this.language.language() === "ar" ? "الكتالوج" : "Catalog");
